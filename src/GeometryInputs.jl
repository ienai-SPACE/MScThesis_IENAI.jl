...
function GeomInputs(Vrel_v, VdirFlag, convexFlag)
    ...
    #load the mesh
    mesh = load("C:\\Users\\danie\\Documents\\UC3M\\IENAI internship\\CAD\\sphereMesh_1036lmns.obj")

    MeshVerticesCoords = finputMesh(mesh)

    if convexFlag == 0
        rmax = maximum(MeshVerticesCoords[:, :]) * 1.1          #radius of the circular plane from where rays originate
