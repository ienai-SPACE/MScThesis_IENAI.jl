...
import SatelliteToolboxAtmosphericModels as STATM
using StaticArrays
...
function GasStreamProperties(JD, alt, g_lat, g_long, f107A, f107, ap)
    ...
    nrlmsise00_output = STATM.AtmosphericModels.nrlmsise00(JD, alt, g_lat, g_long, f107A, f107, ap; include_anomalous_oxygen=true)
...
"""
    GasStreamProperties(nrlmsise00_output::SatelliteToolbox.Nrlmsise00Output)
...
function GasStreamProperties(nrlmsise00_output)
    ...
    den_v = @SVector [out.He_number_density, out.O_number_density, out.N2_number_density, out.O2_number_density, out.H_number_density, out.N_number_density]
    C = @. den_v * m_v / (NA * 1000 * nrlmsise00_output.total_density)
    ...
    den = sum(den_v)
    mmean = num / den      #[g/mol]
    GasStreamProperties(C, PO, mmean, nrlmsise00_output.temperature)
end
